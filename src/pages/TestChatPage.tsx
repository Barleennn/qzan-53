...
именуемым в дальнейшем "Сотрудник".`);
  ...
  return (
    <>
      <div className="relative min-h-screen flex flex-col"> {/* Removed pt-[64px] padding */}
        {/* Main content wrapper with smooth transition */}
        <div className={`
          flex-1 transition-all duration-300 ease-in-out
          ${isFullscreen ? 'pr-[50vw]' : ''}
        `}>
          <div className={`
            max-w-[1440px] mx-auto px-4 py-8
            transition-all duration-300 ease-in-out
            ${isFullscreen ? 'opacity-50' : 'opacity-100'}
          `}>
            <div className="w-full max-w-[814px] mx-auto">
                ...
                <div className="w-[814px] rounded-[9px] p-6 bg-white border border-[#9898BF] shadow-[0px_7px_4px_rgba(0,0,0,0.05)]">
                  <div className="flex items-center justify-between gap-3 mb-4">
                    ...
                    <button 
                      ...
                      className="text-[#9898BF] hover:text-[#202295] transition-colors duration-200"
                    >
                      {isFullscreen 
                      ? <Minimize2 className="w-[25px] h-[25px]" />
                      : <Maximize2 className="w-[25px] h-[25px]" />
                      }
                    </button>
                  </div>
                  <div className="pr-6">
              ...
              <div className={`
                mt-2 mb-6 flex items-center gap-4
                transition-all duration-300
                ${isFullscreen ? 'opacity-50 pointer-events-none' : ''}
              `}>
              ...
              <div className={`
                mb-6 font-roboto text-[18px] leading-[21px] text-black text-left
        ...
        <div className={`
          ...
          ${isFullscreen ? 'pr-[50vw] opacity-50' : 'opacity-100'}
        `}>
          <div className="w-[814px] h-[56px] flex items-center rounded-[9px] px-3 
            bg-white/80 backdrop-blur-sm shadow-lg border border-[#9898BF]
            hover:bg-white transition-all duration-200">
            <PaperclipIcon className="w-[28px] h-[28px] text-[#202295] ml-2" />
            <input
              type="text"
              placeholder="Составь договор NDA для сотрудника компании"
              className="flex-1 h-full px-4 text-[16px] font-light text-[#9898BF] focus:outline-none font-roboto bg-transparent"
            />
        ...
        {isFullscreen && (
          <div className={`
            fixed top-0 right-0 w-[50vw] h-screen
            bg-white border-l border-[#9898BF] shadow-lg
            transition-all duration-300 ease-in-out z-[100]
            animate-expand-sidebar origin-right
              ...
              <div className="absolute top-4 right-4 z-[100] flex items-center gap-4">
                {isEditing ? (
                  <button 
                    onClick={handleSave}
                    className="text-[#9898BF] hover:text-[#202295] transition-colors duration-200 
                      flex items-center gap-2 border border-[#9898BF] rounded-md px-3 py-1
                      bg-white shadow-md"
                  >
                ...
                <button 
                  onClick={toggleFullscreen}
                  className="text-[#9898BF] hover:text-[#202295] transition-colors duration-200 
                    flex items-center gap-2 bg-white border border-[#9898BF] rounded-md px-3 py-1
                    shadow-md"
                >
                ...
                {isEditing ? (
                  <div className="h-full animate-expand-editor origin-top">
                    <textarea
                      ...
                      onBlur={handleSave}
                      className="w-full h-[calc(100vh-120px)] p-4 text-[18px] leading-[21px] text-black
                        border border-[#9898BF] rounded-md focus:outline-none 
                        focus:border-[#202295] resize-none font-roboto
                        transition-all duration-300 ease-in-out
                        bg-white shadow-lg"
                      autoFocus
                    />
                  ...
                  <div className="min-h-[calc(100vh-120px)] text-[18px] leading-[21px] text-black 
                    space-y-4 text-left whitespace-pre-wrap
                    transition-all duration-300 ease-in-out">
                    {contractText}
                  </div>
                )}
        ...
        {isFullscreen && (
          <div className="fixed inset-0 bg-white/50 backdrop-blur-sm z-[90]" />
        )}
