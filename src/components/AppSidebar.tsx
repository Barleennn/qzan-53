...
export function AppSidebar({
  ...
  const menuButton = <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={cn("p-4 hover:bg-gray-50 transition-colors flex items-center justify-center", isMobile ? "fixed top-0 left-0 z-50 h-16" : "absolute top-8 right-4 z-10")}>
      <Menu className="w-6 h-6 text-[#B3B3B3] mx-0 bg-[#000a00]/0" />
    </button>;
  if (isMobile && !isMenuOpen) {
    return menuButton;
  }
